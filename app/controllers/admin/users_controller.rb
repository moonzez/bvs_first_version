class Admin::UsersController < ApplicationController
  ...
  def create
    ...
    if @user.save_with_params(params[:roles], params[:languages], params[:licenses])
      redirect_to admin_users_path, notice: "Nutzer #{ @user.full_name } wurde angelegt"
    else
      render :new
    end
  end
  ...
  def update
    if @user.update_with_params(user_params, params[:roles], params[:languages], params[:licenses])
      redirect_to admin_users_path, notice: "Nutzerdaten für #{ @user.full_name } wurden geändert"
    else
      render :edit
    end
  end
